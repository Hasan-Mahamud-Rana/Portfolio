...
<div class="b300x250">
  <div class="row small-up-1 medium-up-2 large-up-3 banners-panel  columns text-center">
    <?php
      $query = new WP_Query( array( 'order' => 'dsc' , 'post_type' => 'banner' , 'post_status' => 'publish', 'tag' => 'standard-banner-300x250' , 'posts_per_page' => -1  ) );
...
<div class="b160x600">
  <div class="row small-up-2 medium-up-4 large-up-5 banners-panel  columns text-center">
    <?php
      $query = new WP_Query( array( 'order' => 'dsc' , 'post_type' => 'banner' , 'post_status' => 'publish', 'tag' => 'standard-banner-160x600' , 'posts_per_page' => -1  ) );
      if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
        the_title('<h3>', '</h3>');
      if( have_rows('banner') ):
        while( have_rows('banner') ): the_row();
...
<div class="b930x180">
  <div class="row small-up-1 medium-up-1 large-up-1 banners-panel  columns text-center">
    <?php
      $query = new WP_Query( array( 'order' => 'dsc' , 'post_type' => 'banner' , 'post_status' => 'publish', 'tag' => 'standard-banner-930x180' , 'posts_per_page' => -1  ) );
      if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
        the_title('<h3>', '</h3>');
      if( have_rows('banner') ):
        while( have_rows('banner') ): the_row();
