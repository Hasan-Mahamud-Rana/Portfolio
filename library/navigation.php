 ...
 */
if ( ! function_exists( 'bvportfolio_top_bar_r' ) ) {
	function bvportfolio_top_bar_r() {
		wp_nav_menu( array(
			'container'      => false,
			...
			'depth'          => 3,
			'fallback_cb'    => false,
			'walker'         => new bvportfolio_Top_Bar_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'bvportfolio_mobile_nav' ) ) {
	function bvportfolio_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu'           => __( 'mobile-nav', 'bvportfolio' ),
			'menu_class'     => 'vertical menu',
			'theme_location' => 'mobile-nav',
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-accordion-menu>%3$s</ul>',
			'fallback_cb'    => false,
			'walker'         => new bvportfolio_Mobile_Walker(),
		));
	}
}
...
*/
if ( ! function_exists( 'bvportfolio_add_menuclass' ) ) {
	function bvportfolio_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','bvportfolio_add_menuclass' );
}
...
if ( ! function_exists( 'bvportfolio_breadcrumb' ) ) {
	function bvportfolio_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'bvportfolio' ) . ' ' . get_query_var('paged') . '</li>';
